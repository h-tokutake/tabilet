...
var WaypointEditDialog = (function(){
	...
	WaypointEditDialog.prototype.open = function (target) {
		...
		$("#itinerary_edit_screen_waypoint_edit").show().dialog({
			...
			open    : function() {
				...
				if(!mainView.isLoggedIn()) {
					$('.ui-dialog-buttonpane').find('button:contains("行程保存")').button("disable");
					$('.ui-dialog-buttonpane').find('button:contains("行程削除")').button("disable");
				}
			},
			close   : function() {
			...
			buttons : [
								...
								buttons : [
									{
										...
										click: function() {
											if($("#place_edit_place_name").val() == "") {
												mainView.getCommonDialog().error("名称を入植してください。");
												$("#place_edit_place_name").focus();
											} else {
												var msg = '地点 "' + $("#place_edit_place_name").val() + '" の情報を保存します。';
				...
				},
				{
					...
					click: function() {
						...
						if($("#waypoint_edit_place_name").val() == "") {
							mainView.getCommonDialog().error("名称を入力してください。");
							$("#waypoint_edit_place_name").focus();
						} else if(position_dirty_flag) {
							mainView.getCommonDialog().error("地点情報を削除できません。");
						} else {
							var msg = '地点 "' + $("#waypoint_edit_place_name").val() + '" の情報を削除します。';
							mainView.getCommonDialog().confirm(true, msg, function(){
	...
	function ajaxToGetPlaceList () {
		...
		$("#place_list_menu").append('<li>地点情報を読み込んでいます・・・</li>');
