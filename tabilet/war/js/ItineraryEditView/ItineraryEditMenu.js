...
var ItineraryEditMenu = (function() {
	...
	function ItineraryEditMenu (view) {
		...
		$("#itinerary_edit_menu_main_create_itinerary").button({
			...
			var msg = '行程 "' + $("#itinerary_edit_itinerary_summary").val() + '" の変更が破棄されます。よろしいですか？';
		...
		$("#itinerary_edit_menu_main_save_itinerary").button({
			...
			if ($("#itinerary_edit_itinerary_summary").val() === "") {
				mainView.getCommonDialog().error($("#itinerary_edit_itinerary_summary").text() + 'を入力してください。');
				return false;
			}
			if ($("#itinerary_edit_itinerary_deptime").val() === "") {
				mainView.getCommonDialog().error($("#itinerary_edit_itinerary_deptime").text() + 'が入力してください。');
				return false;
			}
		...
		$("#itinerary_list_menu").append('<li>行程一覧を読み込んでいます・・・</li>');
...
//			$("#itinerary_edit_menu_main_show_map").button("disable");
		...
		$.ajax({
			...
			success: function(result){
				if(result.returnCode == "0") {
					mainView.getCommonDialog().ok('保存完了', '行程 "' + itinerary_data_json.itinerary_summary + '" が保存されました。');
	...
	function ajaxToDeleteItineraryData () {
		...
		$.ajax({
			...
			success: function(result){
				if(result.returnCode == "0") {
					mainView.getCommonDialog().ok('削除完了', '行程 "' + itinerary_data_json.itinerary_summary + '" が削除されました。', function(){
						submitForm("itinerary_edit", "get");
					});
				} else {
